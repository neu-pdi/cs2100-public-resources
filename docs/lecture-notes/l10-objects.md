...
## The \_\_eq\_\_() function

Every class also has an `__eq__()` method, which we can also override with out own `__eq__()` method.

Poll: By default, what does `self.__eq__(other)` method do?

1. It returns `True` if `self` and `other` are aliases of each other
2. It returns `True` if all of `self`'s and `other`'s attributes are equal
3. It returns `True` if `self` and `other` are instances of the same type
...
mini = Cat('Mini', 'Rasika')
mini2 = Cat('Mini', 'Rasika')
print(mini == mini2)
```


1. `MUAHAHAHAHHA` // `False`
2. `MUAHAHAHAHHA` // `True`
3. `False`
4. `True`
